...
export default function TicketScreen() {
  ...
  function onGoToTicketPage(id: number) {
    router.push({ pathname: "/(authed)/(tabs)/(tickets)/ticket/[id]", params: { id: id.toString() } });
  }
              ...
              <HStack>
                ...
                >
                  <HStack alignItems="center">
                    <Text numberOfLines={2} adjustsFontSizeToFit fontSize={15} bold>{ticket.event.name}</Text>
                  </HStack>
                  <Text fontSize={12}>{new Date(ticket.event.date).toLocaleString()}</Text>
                  <Text fontSize={15} style={{ flexWrap: 'wrap' }}>{ticket.event.location}</Text>
                </VStack>
                ...
                >
                    <Text fontSize={20} bold>{ticket.entered ? "Used" : "Available"}</Text>
                  {ticket.entered &&
                    <Text style={{ fontSize: 10, paddingLeft: 18, paddingTop: 5 }} mt={12}>{new Date(ticket.updatedAt).toLocaleString()}</Text>
                  }
                </VStack>
              </HStack>
